...
class FeedbackController extends Controller
{
    public function index(Request $request)
    {
        ...
        $positivetoday = (clone $positiveQuery)->whereBetween('created_at', [$startOfDay, $endOfDay])->count();

        $negativeQuery = Feedback:: where('value', 1);
        $negative = $negativeQuery->count();
        $negativetoday = (clone $negativeQuery)->whereBetween('created_at', [$startOfDay, $endOfDay])->count();

        $neutralQuery = Feedback::where('value', 2);
        $neutral = $neutralQuery->count();
        $neutraltoday = (clone $neutralQuery)->whereBetween('created_at', [$startOfDay, $endOfDay])->count();
